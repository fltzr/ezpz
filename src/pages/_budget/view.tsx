...
const ViewBudgetPage = () => {
  ...
  const [isAddBudgetLineItemModalVisible, setIsAddBudgetLineItemModalVisible] =
    useState(false);
  const [budgetLineItemToAdd, setBudgetLineItemToAdd] = useState<BudgetTableItem | null>(
    null
  );
  ...
  const handleAddCategory = () => {
    ...
    addNotification({
      type: 'success',
      message: `Added category: ${categoryToAdd.name}`,
    });
    resetAddCategory();
  };
  ...
  const confirmAddBudgetLineItem = () => {
    ...
    addNotification({
      type: 'success',
      message: `Added budget line item: ${budgetLineItemToAdd.name}`,
    });
  ...
  return (
    <>
        ...
        <Input
          ...
          onChange={(event) =>
            setCategoryToAdd({
              ...
              id: uniqueId(event.detail.value),
              parentId: null,
            })
          }
        />
      ...
      <Modal
        ...
        footer={
          <Box float='right'>
            <Button onClick={() => setIsAddBudgetLineItemModalVisible(false)}>
              Cancel
            </Button>
            <Button variant='primary' onClick={confirmAddBudgetLineItem}>
              Add
            </Button>
          </Box>
        }>
        ...
        <Input
          ...
          value={budgetLineItemToAdd?.name ?? ''}
          onChange={(event) =>
            setBudgetLineItemToAdd({
              ...budgetLineItemToAdd,
              name: event.detail.value,
              id: uniqueId(event.detail.value),
              parentId: budgetLineItemToAdd?.id ?? '',
            })
          }
        />
        <Input
          ...
          onChange={(event) =>
            setBudgetLineItemToAdd({
              ...
              id: budgetLineItemToAdd?.id ?? '',
              name: budgetLineItemToAdd?.name ?? '',
              parentId: budgetLineItemToAdd?.parentId ?? '',
            })
          }
        />
      </Modal>
